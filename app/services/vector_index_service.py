...
from app.services.file_type_handlers import file_handler_registry
from app.services.knowledge_graph_index_service import knowledge_graph_index_service
...
class VectorIndexService:
    ...
    def index_directory(self, directory_path: Optional[str] = None) -> IndexingResult:
        ...
        try:
            ...
            # 获取所有支持的文件
            files = []
            for extension in file_handler_registry.supported_extensions(include_dot=True):
    ...
    def index_single_file(self, file_path: str):
        ...
        try:
            ...
            # 2. 根据文件类型选择处理器，加载并分割文档
            documents = file_handler_registry.load_and_split(path)
            self._ensure_chunk_metadata(normalized_path, documents)
            logger.info(f"文档分割完成: {file_path} -> {len(documents)} 个分片")

            # 3. 写入关键词检索 chunk 索引
            if documents and config.keyword_retrieval_enabled:
                chunk_index_store_service.delete_by_source(normalized_path)
                chunk_index_store_service.upsert_documents(normalized_path, documents)

            # 4. 自动抽取知识图谱。图谱是增强链路，失败不阻断向量入库。
            if documents and config.knowledge_graph_enabled:
                try:
                    self._index_knowledge_graph(normalized_path, documents)
                except Exception as exc:
                    logger.warning(f"知识图谱入库失败，向量入库继续: source={normalized_path}, error={exc}")
    ...
    @staticmethod
    def _ensure_chunk_metadata(source_file: str, documents: list):
        for index, document in enumerate(documents, 1):
            chunk_id = document.metadata.get("_chunk_id")
            if not chunk_id:
                raw = f"{source_file}|{index}|{document.page_content[:200]}"
                chunk_id = hashlib.sha256(raw.encode("utf-8")).hexdigest()

            document.metadata["_source"] = source_file
            document.metadata["_chunk_index"] = index
            document.metadata["_chunk_id"] = chunk_id

    def _index_knowledge_graph(self, source_file: str, documents: list):
        try:
            try:
                asyncio.get_running_loop()
            except RuntimeError:
                asyncio.run(knowledge_graph_index_service.index_documents(source_file, documents))
                return

            with ThreadPoolExecutor(max_workers=1) as executor:
                future = executor.submit(
                    asyncio.run,
                    knowledge_graph_index_service.index_documents(source_file, documents),
                )
                future.result()
        except Exception as exc:
            logger.warning(f"知识图谱入库失败，向量入库继续: source={source_file}, error={exc}")

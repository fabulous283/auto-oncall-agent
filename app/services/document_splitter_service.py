...
class DocumentSplitterService:
    ...
    def split_loaded_documents(self, documents: List[Document], file_path: str = "") -> List[Document]:
        """
        分割已经由 Loader 加载出来的文档列表。

        PDF、Word 这类文件不是普通文本文件，通常先由 LangChain Loader
        解析成 Document，再用递归字符分割器进行二次分片。
        """
        if not documents:
            logger.warning(f"Loader 未解析出文档内容: {file_path}")
            return []
        ...
        try:
            ...
            logger.info(f"Loader 文档分割完成: {file_path} -> {len(final_docs)} 个分片")
            return final_docs
        except Exception as e:
            logger.error(f"Loader 文档分割失败: {file_path}, 错误: {e}")
            raise

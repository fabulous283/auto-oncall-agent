...
class SuperBizAgentApp {
        ...
        try {
            ...
            if (data.code === 200 || data.message === 'success') {
                ...
                if (chatResponse && chatResponse.success) {
                    ...
                    if (chatResponse.handoff && chatResponse.handoff.type === 'aiops') {
                        this.attachAIOpsHandoffActions(assistantMessage, chatResponse.handoff);
                    }
                            ...
                            try {
                                ...
                                if (sseMessage && typeof sseMessage.type === 'string') {
                                    if (sseMessage.type === 'content') {
                                        ...
                                        this.handleStreamComplete(assistantMessageElement, fullResponse);
                                        return;
                                    } else if (sseMessage.type === 'handoff') {
                                        const handoff = sseMessage.data || {};
                                        fullResponse = handoff.message || '\u68c0\u6d4b\u5230\u4f60\u53ef\u80fd\u60f3\u505a\u544a\u8b66/\u6545\u969c\u8bca\u65ad\uff0c\u662f\u5426\u8fdb\u5165 AIOps \u5206\u6790\u6d41\u7a0b\uff1f';
                                        if (assistantMessageElement) {
                                            const messageContent = assistantMessageElement.querySelector('.message-content');
                                            messageContent.innerHTML = this.renderMarkdown(fullResponse);
                                            this.highlightCodeBlocks(messageContent);
                                            this.attachAIOpsHandoffActions(assistantMessageElement, handoff);
                                            this.scrollToBottom();
                                        }
                                        this.handleStreamComplete(assistantMessageElement, fullResponse);
                                        return;
    ...
    // 添加带加载动画的消息
    attachAIOpsHandoffActions(messageElement, handoff) {
        if (!messageElement || !handoff) {
            return;
        }
        ...
        const existingActions = messageContentWrapper.querySelector('.handoff-actions');
        if (existingActions) {
            existingActions.remove();
        }

        const actionsContainer = document.createElement('div');
        actionsContainer.className = 'handoff-actions';

        const confirmBtn = document.createElement('button');
        confirmBtn.className = 'handoff-btn primary';
        confirmBtn.textContent = '\u5f00\u59cb AIOps \u8bca\u65ad';

        const cancelBtn = document.createElement('button');
        cancelBtn.className = 'handoff-btn secondary';
        cancelBtn.textContent = '\u7ee7\u7eed\u666e\u901a\u804a\u5929';
        ...
        confirmBtn.addEventListener('click', async () => {
            ...
            messageElement.dataset.aiopsFromHandoff = 'true';

            const messageContent = messageElement.querySelector('.message-content');
            if (messageContent) {
                messageContent.textContent = '\u5206\u6790\u4e2d...';
            }
            ...
            try {
                await this.sendAIOpsRequest(messageElement);
            } catch (error) {
                console.error('AIOps 诊断失败:', error);
                if (messageContent) {
                    messageContent.textContent = '\u62b1\u6b49\uff0c\u8fdb\u5165 AIOps \u8bca\u65ad\u65f6\u51fa\u73b0\u9519\u8bef\uff1a' + error.message;
                }
            } finally {
                this.isStreaming = false;
                this.updateUI();
    ...
    updateAIOpsMessage(messageElement, response, details) {
        ...
        // 保存到历史记录
        if (messageElement.dataset.aiopsFromHandoff === 'true') {
            const historyLength = this.currentChatHistory.length;
            if (historyLength > 0) {
                this.currentChatHistory[historyLength - 1] = {
                    type: 'assistant',
                    content: response,
                    timestamp: new Date().toISOString()
                };
            ...
            delete messageElement.dataset.aiopsFromHandoff;
        } else {
            this.currentChatHistory.push({
                type: 'assistant',
                content: response,

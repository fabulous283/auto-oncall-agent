...
上传知识库文件会根据扩展名自动选择处理器，目前支持：

- `.md`：Markdown 标题分片 + 递归字符二次分片
- `.txt`：递归字符分片
- `.pdf`：`PyPDFLoader` 按页解析 + 递归字符二次分片
- `.docx`：`Docx2txtLoader` 提取 Word 文本 + 递归字符分片
